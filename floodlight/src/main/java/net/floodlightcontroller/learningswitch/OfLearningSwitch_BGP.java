...
public class OfLearningSwitch_BGP
implements IFloodlightModule, ILearningSwitchService, IOFMessageListener, IOFSwitchListener {
	...
	protected static Logger log = LoggerFactory.getLogger(OfLearningSwitch_BGP.class);
	// Module dependencies
	...
	private void writeFlowMod(IOFSwitch sw, OFFlowModCommand command, OFBufferId bufferId,
		...
		fmb.setMatch(match);
		fmb.setCookie((U64.of(OfLearningSwitch_BGP.LEARNING_SWITCH_COOKIE)));
		fmb.setIdleTimeout(OfLearningSwitch_BGP.FLOWMOD_DEFAULT_IDLE_TIMEOUT);
		fmb.setHardTimeout(OfLearningSwitch_BGP.FLOWMOD_DEFAULT_HARD_TIMEOUT);
		fmb.setPriority(OfLearningSwitch_BGP.FLOWMOD_PRIORITY);
	...
	private Command processFlowRemovedMessage(IOFSwitch sw, OFFlowRemoved flowRemovedMessage) {
		if (!flowRemovedMessage.getCookie().equals(U64.of(OfLearningSwitch_BGP.LEARNING_SWITCH_COOKIE))) {
			return Command.CONTINUE;
		}
		if (log.isTraceEnabled()) {
	...
	public void switchAdded(DatapathId dpid) {				
		...
		for(int i=0; i < 2; i++) {
			...
			if (i == 0)
				match.setExact(MatchField.TCP_SRC, BGP_PORT);
			else
				match.setExact(MatchField.TCP_DST, BGP_PORT);
