...
public class BGPSecOpenHandle {
	...
	public static StringBuilder MY_DEFAULT_OPEN_HEADER = new StringBuilder()
			...
			.append(BGPSecUtils.decToHexWithPad(BGPSecDefs.DEFAULT_HOLD_TIME, 4)).append(BGPSecDefs.MY_ID);

	/*
	 * RFCs 5492;8126, Optional Parameters format IANA Capability Code
	 * https://www.iana.org/assignments/capability-codes/capability-codes.xhtml 0 1
	...
	public static StringBuilder OPEN_MSG_TO_REPLY = new StringBuilder().append(BGPSecDefs.HEADER_MARKER_HEX)
			.append(MY_DEFAULT_OPEN_HEADER).append(MY_OPEN_OPTIONAL_PARAM);
	// .append(BGPSecDefs.KEEPALIVE_MSG);

	/*
	 * MSG HEADER (01-04-#ASN-##HT-@@@ID) #ASN: AS Number; ##HT: Hold Time; @@ID of
	 * speaker
	 * 
	 * RETURNED CODES 01: Peer is not authorized to keep a BGP session 02: The
	 * minimum length of message is wrong 03: Incompatible BGP Version 04: Hold Time
	 * is out-off the specified
	 */
	public static byte[] parseMsg(byte[] msg, String thread) {
		...
		if (BGPSecMain.containsPeer(rmtPeerAsn)) {
			if (BGPSecMain.getAuthPeersId(rmtPeerAsn).equals(rmtPeerId)) {
				log.info("OPEN message, error: ASN " + rmtPeerAsn + " is a authorized peer, but ID "
						+ BGPSecUtils.ipHexToDec(rmtPeerId) + " not match.");
				return new byte[] { 0x01 };
			} else {
				log.info("OPEN message: ASN " + rmtPeerAsn + " from ID " + BGPSecUtils.ipHexToDec(rmtPeerId)
						+ " is a authorized peer.");
			}
		} else {
		...
		// Check the length of message (17 bytes are header mark and the type of
		// message)
		...
		log.info("OPEN message info: Optional Parameters (Capabilities) Lenght: " + optParamLen + ", content: "
				+ BGPSecUtils.bytesToHex(optParam));
		parseCapabilities(optParamLen, optParam, rmtPeerAsn);

		/*
		 * Calculates the total message length (18 is value of header mark + 2 bytes of
		 * message length)
		 */
		String msgLen = BGPSecUtils
				.decToHexWithPad(((MY_DEFAULT_OPEN_HEADER.length() + MY_OPEN_OPTIONAL_PARAM.length()) / 2) + 18, 4);
		StringBuilder buildMsg = new StringBuilder().append(BGPSecDefs.HEADER_MARKER_HEX).append(msgLen)
				.append(MY_DEFAULT_OPEN_HEADER).append(MY_OPEN_OPTIONAL_PARAM).append(BGPSecDefs.KEEPALIVE_MSG);
			if (BGPSecMain.sessionData.containsPeer(rmtPeerId)) {
				log.info("Há ID...........");
			} else {
				BGPSecClientFSMHandle.setSessionPar("asn" + thread, rmtPeerAsn);
				BGPSecClientFSMHandle.setSessionPar("id" + thread, rmtPeerId);
				BGPSecMain.sessionData.setAllParameters(BGPSecClientFSMHandle.getSessionPar(),
						BGPSecClientFSMHandle.getClientIpAddr());
			}
		// There are not any errors in OPEN message. Reply with other OPEN + a
		// concatenated KEEPALIVE
	...
	public static void parseCapabilities(int optParamLen, byte[] optParam, int peer) {
		...
		while (count < optParamLen) {
			...
			while (count < (newCap + parLen)) {
				...
				count++;
				// System.out.println("count: " + count + ", parLen: " + parLen + ", capType: "
				// + capType +", capLen: " + capLen);
				if (capLen != 0) {
					capValue = BGPSecUtils.bytesToHex(BGPSecUtils.subByte(optParam, count, capLen));
				}
				if (bgpDefs.optCapCodes.containsKey(capType)) {
					log.info("New CAPABILITY type for peer " + peer + " is: " + bgpDefs.optCapCodes.get(capType)
							+ ", and value is: " + capValue);
				}
				count += capLen + 1;
			}
		}
