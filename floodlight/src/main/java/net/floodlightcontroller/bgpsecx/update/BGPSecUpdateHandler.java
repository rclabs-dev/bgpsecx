...
import net.floodlightcontroller.bgpsecx.general.*;;

public class BGPSecUpdateHandler extends BGPSecDefs {
	protected static Logger log = LoggerFactory.getLogger(BGPSecUpdateHandler.class);
	...
	public static byte[] processBGPPkt(byte[] bgpMsg, IPv4Address speakerIP) {
		...
		log.info("The Number of BGP messages in stream are: " + totUpdtMsg);
		...
		for (int i = 0; i < totUpdtMsg; i++) {
			...
			if (msg == UPDATE && msgLen) {
				...
				if (attrData.containsKey(1)) {
					...
					log.info("WITHDRAW route(s) prefix chain: " + prefChain);
				...
				if (attrData.containsKey(8)) {
					...
					for (int j = 0; j < prefixes.size(); j += 2) {
						...
						log.info("RESULT for NLRI routes Validadtor is: *** "
								+ roaResult + " ***");
						totPrefixNLRI++;
						if (roaResult)
				...
				if (totNLRI != 0 || totWITH != 0) {
					...
					String msgLength = String.format("%04X",
							((payTemp.length() / 2) + 18));
					payTemp = BGPSecDefs.HEADER_MARKER_HEX + msgLength + payTemp;
			...
			log.info("Last time of msg verification: " + elapsedTime);
	...
	public static boolean containsMark(byte[] msg) {
		...
		if (msg.length < MIN_MSG_LENGTH[3]) {
			return false;
		}
	...
	public static boolean checkMsgLength(byte[] msg, int minLen) {
		int len = msg.length;
		if (len < MIN_MSG_LENGTH[minLen] || len > MAX_LENGTH_MSG) {
			return false;
		}
		return true;
	}
